...
public class ConnectNNode {
    ...
    // these need to keep in mind disjoint plays:       XX_X   should be counted as 3in a row
    // if there is an empty space in between
    // dropping a peice that forms 2 rows of N-1 should get a higher value than only 1 row
    ...
    private int popOutHeuristic() {
            return 0;
        //  if there is N-1 in a row below a peice which is in a location to drop down for win
        // this should be a function that calls the rowHeuristic and then checks to see if 
